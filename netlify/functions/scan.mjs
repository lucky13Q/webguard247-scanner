// WebGuard247 — scan endpoint (Netlify Function), v3.
...
//
// v3 adds a SERVER-SIDE paywall: the step-by-step fixes (and the problem detail)
// are stripped out of the response unless the caller is a verified paying user.
// Free users and anonymous (public homepage) callers get grade + statuses +
// check names only. The fixes never leave the server for them.
...
// --- paywall config ---
// SUPABASE_URL isn't secret (it's in the client too), so we fall back to the literal.
// SERVICE_ROLE_KEY is a secret env var already set in Netlify — it lets us read the
// authoritative plan from the database, bypassing row-level security.
const SUPABASE_URL = process.env.SUPABASE_URL || 'https://uhyuvlqxlfvolmykrmce.supabase.co';
const SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';
const PAID_PLANS = new Set(['guide', 'pro', 'agency']);

// Look at the caller's "membership card" (a Supabase login token) and return their
// real plan. Anything unverified, anonymous, or failing safely returns 'free' — so
// the default is always locked, never accidentally open.
async function resolvePlan(event) {
  try {
    ...
    // 1) Verify the token and find out who it belongs to (Supabase does the check).
    ...
    // 2) Read the authoritative plan straight from the database (service role
    //    bypasses RLS, so this is the true value the user can't tamper with).
...
// Strip the paid content (the fix + the problem detail) out of a report so a free
// caller literally never receives it. Grade, score, statuses and check names stay.
function lockReport(report) {
  if (!report || typeof report !== 'object') return report;
  const strip = (f) => {
    if (!f || typeof f !== 'object') return f;
    const { recommendation, detail, ...rest } = f;
    return { ...rest, locked: true };
  };
  const out = { ...report, locked: true };
  if (Array.isArray(out.findings)) out.findings = out.findings.map(strip);
  if (out.categories && typeof out.categories === 'object') {
    const cats = {};
    for (const k in out.categories) {
      cats[k] = Array.isArray(out.categories[k]) ? out.categories[k].map(strip) : out.categories[k];
    }
    out.categories = cats;
  }
  return out;
...
const CORS = {
  ...
  // Authorization is allowed so the logged-in dashboard can send its login token.
...
export const handler = async (event) => {
  ...
  // Work out the caller's plan in parallel with the scan (never throws; defaults to 'free').
  const planPromise = resolvePlan(event);

  // Helper: apply the paywall to whatever report we end up with, then respond.
  const respond = async (report) => {
    const plan = await planPromise;
    const finalReport = PAID_PLANS.has(plan) ? report : lockReport(report);
    return { statusCode: 200, headers: CORS, body: JSON.stringify(finalReport) };
  };
